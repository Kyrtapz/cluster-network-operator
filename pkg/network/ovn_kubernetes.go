...
func renderOVNKubernetes(conf *operv1.NetworkSpec, bootstrapResult *bootstrap.BootstrapResult, manifestDir string,
	...
	var joinSubnet, transitSwitchSubnet, internalMasqueradeSubnet []string
	...
	if c.GatewayConfig != nil && c.GatewayConfig.IPv4.InternalMasqueradeSubnet != "" {
		...
		internalMasqueradeSubnet = append(internalMasqueradeSubnet, c.GatewayConfig.IPv4.InternalMasqueradeSubnet)
	}
	if c.GatewayConfig != nil && c.GatewayConfig.IPv6.InternalMasqueradeSubnet != "" {
		...
		internalMasqueradeSubnet = append(internalMasqueradeSubnet, c.GatewayConfig.IPv6.InternalMasqueradeSubnet)
	}
	...
	err = setOVNObjectAnnotation(objs, names.JoinSwitchSubnetAnnotation, strings.Join(joinSubnet, ","))
	if err != nil {
		return nil, progressing, errors.Wrapf(err, "failed to set join switch subnet %s annotation on ovnkube daemonset and deployment", strings.Join(joinSubnet, ","))
	}

	err = setOVNObjectAnnotation(objs, names.TransitSwitchSubnetAnnotation, strings.Join(transitSwitchSubnet, ","))
	if err != nil {
		return nil, progressing, errors.Wrapf(err, "failed to set transit switch subnet %s annotation on ovnkube daemonset and deployment", strings.Join(transitSwitchSubnet, ","))
	}

	err = setOVNObjectAnnotation(objs, names.InternalMasqueradeSubnetAnnotation, strings.Join(internalMasqueradeSubnet, ","))
	if err != nil {
		return nil, progressing, errors.Wrapf(err, "failed to set masquerae subnet %s annotation on ovnkube daemonset and deployment", strings.Join(internalMasqueradeSubnet, ","))
	}
...
func validateOVNKubernetesSubnets(conf *operv1.NetworkSpec) error {
	...
	oc := conf.DefaultNetwork.OVNKubernetesConfig
	// Validate whether join and transit switch CIDRs are from same IP family
	// as clusterNetwork. Also validate whether provided CIDRs have enough IPs to
	// allocate to all nodes scheduleable as per cluterNetwork CIDR and hostPrefix.
	...
	if oc.V4InternalSubnet != "" {
		...
		if err := validateOVNKubernetesSubnet("v4InternalSubnet", oc.V4InternalSubnet, &pool, conf.ClusterNetwork); err != nil {
			out = append(out, err)
		}
	}
	...
	if oc.V6InternalSubnet != "" {
		...
		if err := validateOVNKubernetesSubnet("v6InternalSubnet", oc.V6InternalSubnet, &pool, conf.ClusterNetwork); err != nil {
			out = append(out, err)
		}
	}
	...
	if oc.IPv4 != nil {
		if oc.IPv4.InternalJoinSubnet != "" {
			if !cnHasIPv4 {
				out = append(out, errors.Errorf("v4InternalJoinSubnet %s and ClusterNetwork must have matching IP families", oc.IPv4.InternalJoinSubnet))
			}
			if err := validateOVNKubernetesSubnet("v4InternalJoinSubnet", oc.IPv4.InternalJoinSubnet, &pool, conf.ClusterNetwork); err != nil {
				out = append(out, err)
			}
		}
	...
	if oc.IPv6 != nil {
		if oc.IPv6.InternalJoinSubnet != "" {
			if !cnHasIPv6 {
				out = append(out, errors.Errorf("v6InternalJoinSubnet %s and ClusterNetwork must have matching IP families", oc.IPv6.InternalJoinSubnet))
			}
			if err := validateOVNKubernetesSubnet("v6InternalJoinSubnet", oc.IPv6.InternalJoinSubnet, &pool, conf.ClusterNetwork); err != nil {
				out = append(out, err)
			}
		}
	...
	if oc.V4InternalSubnet != "" && oc.IPv4 != nil {
		if oc.V4InternalSubnet != oc.IPv4.InternalJoinSubnet {
	...
	if oc.V6InternalSubnet != "" && oc.IPv6 != nil {
		if oc.V6InternalSubnet != oc.IPv6.InternalJoinSubnet {
	...
	if oc.GatewayConfig != nil {
		if oc.GatewayConfig.IPv4.InternalMasqueradeSubnet != "" {
			...
			if err := validateOVNKubernetesSubnet("v4InternalMasqueradeSubnet", oc.GatewayConfig.IPv4.InternalMasqueradeSubnet, &pool, conf.ClusterNetwork); err != nil {
				out = append(out, err)
			}
		}
		if oc.GatewayConfig.IPv6.InternalMasqueradeSubnet != "" {
			...
			if err := validateOVNKubernetesSubnet("v6InternalMasqueradeSubnet", oc.GatewayConfig.IPv6.InternalMasqueradeSubnet, &pool, conf.ClusterNetwork); err != nil {
				out = append(out, err)
			}
		}
	}
...
// Check subnet length, IP family and overlapping with other subnets
func validateOVNKubernetesSubnet(name, subnet string, otherSubnets *iputil.IPPool, cn []operv1.ClusterNetworkEntry) error {
	out := []error{}
	_, cidr, err := net.ParseCIDR(subnet)
	if err != nil {
		out = append(out, errors.Errorf("%s is invalid: %s", name, err))
	} else if name != "v4InternalMasqueradeSubnet" && !utilnet.IsIPv6CIDRString(subnet) && !isV4NodeSubnetLargeEnough(cn, subnet) {
		out = append(out, errors.Errorf("%s %s is not large enough for the maximum number of nodes which can be supported by ClusterNetwork", name, subnet))
	} else if name != "v6InternalMasqueradeSubnet" && utilnet.IsIPv6CIDRString(subnet) && !isV6NodeSubnetLargeEnough(cn, subnet) {
		out = append(out, errors.Errorf("%s %s is not large enough for the maximum number of nodes which can be supported by ClusterNetwork", name, subnet))
	}
	// If both InternalSubnet and InternalJoinSubnet has been added then CNO should not exhibit
	// an overlap error. So, avoiding addition of InternalSubnet to otherSubnets IPPpool
	if name != "v4InternalSubnet" && name != "v6InternalSubnet" {
		if err := otherSubnets.Add(*cidr); err != nil {
			out = append(out, errors.Errorf("Whole or subset of %s CIDR %s is already in use: %s", name, subnet, err))
		}
	}
	return kerrors.NewAggregate(out)
}
